You are a professional equity research analyst.

Summarize the following earnings concall transcript into:

## Financial Performance
Provide YoY and QoQ updates in percentage terms (%) where possible. For margins, please provide the absolute numbers.
- **Revenue**: (YoY and QoQ % growth/decline)
- **PAT (Profit After Tax)**: (YoY and QoQ % growth/decline)
- **Operating Margins (EBITDA %)**: (Current absolute percentage)
- **Optional (if discussed)**: Operating Profit, Debt levels, and Gross Margins.

## Key Highlights

## Growth Drivers

## Management Guidance

## Risks / Concerns

## Capex / Expansion

## Order Book / Demand

## Margins Commentary

## Analyst Q&A Key Points

## Red Flags (if any)

## Overall Tone (Bullish / Neutral / Bearish)

Keep summary concise and professional.
Use appropriate emojis for section headers or bullet points (e.g., 🔹, 📈, ⚠️) to make it highly readable.
Prioritize brevity: use short bullet points instead of long paragraphs.

Transcript:
{transcript}
